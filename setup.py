...
# Library name
libname = "linear_segment"
# Build type
build_type = "optimized"
#build_type="debug"

# Descriptions of package
SHORTDESC = "Python package for Bayesian Change Point Segmentation"
#DESC = """A python package wrapper for a C implementation of Bayesian Change Point Segmentation."""
...
my_include_dirs = [".", np.get_include(), ailist.get_include()]
...
ext_module_bcp = declare_cython_extension( "linear_segment.bcpseg.bcp_segment", use_math=False, use_openmp=False , include_dirs=my_include_dirs )
ext_module_cbs = declare_cython_extension( "linear_segment.cbseg.cbseg", use_math=False, use_openmp=False , include_dirs=my_include_dirs )

# this is mainly to allow a manual logical ordering of the declared modules
cython_ext_modules = [ext_module_bcp, ext_module_cbs]
...
setup(
    ...
    url = "https://github.com/kylessmtih/linear_segment",
    ...
    provides = ["linear_segment"],
    keywords = ["cython bayesian change point segmentation c"],
    ext_modules = my_ext_modules,
    packages = ["linear_segment"],
    package_data={'linear_segment': ['*.pxd', '*.pyx', '*.c', '*.h']},
    # Disable zip_safe
    zip_safe = False,
